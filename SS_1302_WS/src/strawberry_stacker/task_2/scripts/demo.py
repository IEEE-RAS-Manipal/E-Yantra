...
class SetpointPosition:
    ...
    def _init_(self):
        ...
        self.sub = rospy.Subscriber(
            mavros.get_topic("local_position", "pose"), SP.PoseStamped, self.reached
        )
    ...
    def reached(self, topic):
        ...
        if (
            is_near("X", topic.pose.position.x, self.x)
            and is_near("Y", topic.pose.position.y, self.y)
            and is_near("Z", topic.pose.position.z, self.z)
        ):
            self.done = True
            self.done_evt.set()
