...
class Drone:
    ...
    class DroneControl:
        ...
        def quad_algo(self, quad_x, quad_y, curve_switch, package_pos: list):
            '''
            This function adjusts the position of the drone for precise landing using quadrant logic. The inputs quad_x and quad_y are the coordinates of the point to fixate on. curve_switch lets the user switch between different curves for smoother landing
            '''
            ...
            vel = [0.0, 0.0, 0.0]
            if curve_switch == 1:  # Suitable for higher altitudes
                ...
                # Tweaking velocity of the drone using the exp(0.6x-1.5) function
                vel[0] = exp(
                    0.6
                    * abs(
                        package_pos[0] -
        ...
        def drone_package_pick(self, package_pos: list) -> None:
            ...
            # Going back to the point where aruco was in sight
            ...
            while self.drone_monitor.current_state.armed:
                # Real-time position of ArUco marker
                [aruco_cx, aruco_cy] = self.drone_monitor.aruco_centre[0][:]
                while not ((aruco_cx in range(170, 230)) and (aruco_cy in range(170, 230))) and self.drone_monitor.current_pose.pose.position.z > 2:
                    self.quad_algo(200, 200, 1, package_pos)
                print("out of 1st quad algo...")
                package_pos[0] = self.drone_monitor.current_pose.pose.position.x
                package_pos[1] = self.drone_monitor.current_pose.pose.position.y + 0.3
                package_pos[2] = 2
                self.drone_set_goal(package_pos, True, True)

                while not ((aruco_cx in range(190, 210)) and (aruco_cy in range(310, 330))) and self.drone_monitor.current_pose.pose.position.z > 0.2:
                    self.quad_algo(200, 320, 2, package_pos)

                self.drone_monitor.goal_vel.linear.x = 0
                self.drone_monitor.goal_vel.linear.y = 0
                print("out of 2nd quad algo...")
                self.drone_shutdown()
                RATE.sleep()
