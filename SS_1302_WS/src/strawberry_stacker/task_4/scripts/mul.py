...
from gazebo_ros_link_attacher.srv import *  # Gripper commands
...
class Drone:
    def __init__(self, drone_num) -> None:
        pass


class DroneMonitor(Drone):
    """
    ...
    def pose_callback(self, curr_pose: PoseStamped) -> None:
        ...
        if (
            ...
            drone1.drone_control.reached = True
            self.event_log.set()
...
class DroneControl(Drone):
    ...
    def __init__(self, n) -> None:
        ...
        self.arm_service = rospy.ServiceProxy(
            "/edrone0/mavros/cmd/arming", CommandBool)
        rospy.wait_for_service(f"/edrone{n}/mavros/cmd/arming")
        # Drone Flight Mode Service
        self.set_mode_service = rospy.ServiceProxy(
            "/edrone0/mavros/set_mode", SetMode)
        ...
        try:
            ...
            self.data_stream = threading.Thread(
                target=self.drone_data_stream)
    ...
    def drone_startup(self) -> None:
        ...
        while not drone1.drone_monitor.current_state.armed:
            drone1.drone_control.drone_arming()
            RATE.sleep()
        rospy.logwarn("Drone Armed!")

        # Switching state to OFFBOARD
        rospy.loginfo("Turning on OFFBOARD Mode...")
        while not drone1.drone_monitor.current_state.mode == "OFFBOARD":
            drone1.drone_control.drone_flight_mode("OFFBOARD")
            RATE.sleep()
    ...
    def drone_shutdown(self) -> None:
        ...
        while not drone_monitor.current_state.mode == "AUTO.LAND":
            drone_control.drone_flight_mode("AUTO.LAND")
            RATE.sleep()
        # Waiting for drone to disarm after landing
        while drone_monitor.current_state.armed:
            RATE.sleep()
            ...
            try:
                if not self.stream_switch:  # Position setpoints
                    self.position_publisher.publish(
                        drone_monitor.goal_pose)
                else:  # Velocity setpoints
                    self.velocity_publisher.publish(drone_monitor.goal_vel)
                RATE.sleep()
    ...
    def drone_set_goal(self, goal_pose: list, override: bool = False) -> None:
        ...
        if self.drone_monitor.aruco_check and not override:
            package_pos = [
                drone_monitor.current_pose.pose.position.x,
                drone_monitor.current_pose.pose.position.y,
            ]
            rospy.loginfo(
    ...
    def drone_package_pick(self, package_pos: list) -> None:
        ...
        while drone_monitor.current_state.armed:
            ...
            [aruco_cx, aruco_cy] = drone_monitor.aruco_centre[0][:]
            ...
            if (aruco_cx in range(150, 170)) and (aruco_cy in range(230, 250)):
                package_pos[0] = drone_monitor.current_pose.pose.position.x
                package_pos[1] = drone_monitor.current_pose.pose.position.y
                drone_monitor.goal_vel.linear.z = -0.4

            # Positional adjustment based on relative quadrant-based approach
            # First quadrant (top-right)
            if aruco_cx > 160 and aruco_cy < 240:
                drone_monitor.goal_vel.linear.x = vel[0]
                drone_monitor.goal_vel.linear.y = vel[1]
            ...
            if drone_monitor.current_pose.pose.position.z < -0.2:
                drone_monitor.goal_vel.linear.z = -0.15
            if drone_monitor.current_pose.pose.position.z < -0.3:
        ...
        while not drone_monitor.gripper_state:
            print(drone_monitor.gripper_state)
            RATE.sleep()
...
if __name__ == "__main__":
    try:
        ...
        drone1.drone_monitor = DroneMonitor(1)
        drone1.drone_control = DroneControl(1)

        # Initialising publishers and subscribers
        # Position publisher
        drone1.position_publisher = rospy.Publisher(
            "/edrone0/mavros/setpoint_position/local", PoseStamped, queue_size=10
        )
        # Velocity publisher
        drone1.velocity_publisher = rospy.Publisher(
            "/edrone0/mavros/setpoint_velocity/cmd_vel_unstamped", Twist, queue_size=10
        )
        rospy.loginfo("Publishers initialised.")

        # State subscriber
        drone1.state_subscriber = rospy.Subscriber(
            "/edrone0/mavros/state", State, drone1.drone_monitor.state_callback, queue_size=10
        )
        #   Pose subscriber
        drone1.pose_subscriber = rospy.Subscriber(
            "/edrone0/mavros/local_position/pose",
            PoseStamped,
            drone1.drone_monitor.pose_callback,
            queue_size=10,
        )
        # Drone Gripper Subscriber
        drone1.gripper_subscriber = rospy.Subscriber(
            "/edrone0/gripper_check", String, drone1.drone_monitor.gripper_callback
        )
        # ArUco detection camera subscriber
        drone1.cam_subscriber = rospy.Subscriber(
            "/edrone0/camera/image_raw", Image, drone1.drone_monitor.aruco_callback
        )
        ...
        if drone1.drone_monitor.gripper_state:
            drone1.drone_control.drone_package_place(
                setpoints[1])  # Placing package
