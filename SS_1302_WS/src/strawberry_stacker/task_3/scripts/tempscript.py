...
class offboard_control:
    ...
    def set_mode_function(self, mode):
        ...
        # and print fail message on failure
            ...
            # and print fail message on failure
            print(
                "service set_mode call failed: %s. {0} Mode could not be set. Check that GPS is enabled %s"
                % e,
                mode,
            )
...
class stateMoniter:
    ...
    def proximity_checker(self, x, y, z):
        ...
        offset = 0.2  # offset is in metres
        desired = np.array((x, y, z))
        actual = np.array(
            (
                self.from_drone.pose.position.x,
...
def main():
    ...
    gripper_check = rospy.Subscriber("/gripper_check", String, stateMt.grip_check)
