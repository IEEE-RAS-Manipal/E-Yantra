...
class ImageProc():
    ...
    def marker_pos(self):
        ...
        '''
        for key, _ in self.detected_aruco_markers.items():
            # Calculating relevante point coordinates
            x_0, y_0 = map(int, self.detected_aruco_markers[key][0][0])
            x_1, y_1 = map(int, self.detected_aruco_markers[key][0][1])
            x_2, y_2 = map(int, self.detected_aruco_markers[key][0][2])
...
if __name__ == '__main__':
    ...
    try:
        while not rospy.is_shutdown(): # Will run till node active
            ...
            image_proc_obj.detected_aruco_markers = ImageProc.detect_aruco(
                image_proc_obj.img)
